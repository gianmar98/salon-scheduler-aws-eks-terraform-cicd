...
`appointments-app/buildspecs/buildspec_buildimage.yml`, and CodeBuild reads it from the cloned
...
|---|---|---|
| `buildimages` | `buildimages.xml` | JUnit, written by the `xmlrunner` test runner |
...
| Name | Type | Note |
|---|---|---|
| `buildimage_codebuild_project_name` | string | env-suffixed by the caller; changing it **replaces the project** and renames the role, both policies, the log group, and the report groups |
| `buildimage_codebuild_codeconnection_arn` | string | the CodeConnections connection to GitHub; the role is granted `UseConnection` on it |
| `buildimage_codebuild_source_location` | string | `https://github.com/<owner>/<repo>` — a trailing `/tree/<branch>` breaks the clone |
| `buildimage_codebuild_source_version` | string | branch, tag, or commit |
| `buildimage_codebuild_buildspec` | string | path from the **repo root**, not from this module |
| `buildimage_codebuild_image` | string | must ship the Python the buildspec requests |
...
| `buildimage_codebuild_log_retention_days` | number | CloudWatch retention; `0` keeps logs forever |
| `buildimage_codebuild_artifact_bucket_name` | string | the pipeline's artifact bucket; the role reads the source zip from it |
| `buildimage_codebuild_webhook_branch_pattern` | string | **unused** while `webhook.tf` is commented out |
| `buildimage_codebuild_webhook_file_path_pattern` | string | **unused** while `webhook.tf` is commented out |

## What it creates

| Resource | Purpose |
|---|---|
| `aws_codebuild_project.buildimage` | the build project |
| `aws_iam_role.buildimage` | service role, trusted by `codebuild.amazonaws.com` |
| `aws_iam_policy.buildimage_base` + attachment | logs, artifact buckets, reports |
| `aws_iam_policy.buildimage_codeconnections` + attachment | source clone through CodeConnections |
| `aws_cloudwatch_log_group.buildimage` | `/aws/codebuild/<project>`, with retention |
| `aws_codebuild_webhook.buildimage` | **commented out** — see "Who triggers this project" below |
| `aws_codebuild_report_group.buildimage` | `buildimages` (TEST) and `NewCoverage` (CODE_COVERAGE) |
...
| Name | Value |
|---|---|
| `buildimage_codebuild_project_name` | project name |
| `buildimage_codebuild_project_arn` | project ARN |
| `buildimage_codebuild_service_role_arn` | service role ARN |
| `buildimage_codebuild_log_group_name` | log group name |
...
```bash
aws iam get-role --role-name codebuild-buildimage-dev-service-role
aws iam list-attached-role-policies --role-name codebuild-buildimage-dev-service-role
aws iam get-policy-version --policy-arn <arn> --version-id <default>
aws logs describe-log-groups --log-group-name-prefix /aws/codebuild/buildimage-dev
```

`iam.tf` was then written by hand to match, and `import` blocks pointed at it. Writing
first is what let the names stay derived from `var.buildimage_codebuild_project_name`
...
- **Renaming the project renames almost everything.** The role, both policies, the log
  ...
  `buildimage_codebuild_project_name`, so changing it replaces seven objects, not one.
